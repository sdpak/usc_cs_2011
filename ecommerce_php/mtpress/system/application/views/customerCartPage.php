...
<!------------------------------------------------------------JSP-------------------------------------------------------------->
	...
	<TABLE cellpadding = "2" width = "740" id="dataTable" >
		...
		<TH style="background-color:#C2D4D6;font-size:13px;padding:2mm 2mm 2mm 2mm;border:1px solid;">Update/Remove</TH>
		
		</TR>
				<?php
			   
			    //$session_username = $this->session->userdata('session_username');
				$sql_cartdisplay = "SELECT * FROM cart WHERE username = ?";
    			$cartdisplay = $this->db->query($sql_cartdisplay, array($session_username)); 
				if($cartdisplay->num_rows() > 0):
				   foreach ($cartdisplay->result() as $rowcart):
				?>

		<tr>
		
		<td style ="font-size:13px;padding:2mm 2mm 2mm 2mm;border:1px solid;"><?php echo $rowcart->productid;?>	</td>
		<td style ="font-size:13px;padding:2mm 2mm 2mm 2mm;border:1px solid;"><?php echo $rowcart->productname;?>	</td>
		<td style ="font-size:13px;padding:2mm 2mm 2mm 2mm;border:1px solid;"><?php echo $rowcart->actualprice;?></td>
		<td style ="font-size:13px;padding:2mm 2mm 2mm 2mm;border:1px solid;">	
		<INPUT size = 4  type="text" name="<?=$rowcart->productid?>" value="<?=$rowcart->quantity?>"/></td>
		<td width = 180 style ="font-size:13px;padding:2mm 2mm 2mm 2mm;border:1px solid;text-align:center;">
		<input type="submit" value='Update' style="border-style:none;color:#3A53C4;background-color:#fff;font-size:12px;font-weight:bold;"
		onClick ="javascipt:document.cartform.bookid.value='<?=$rowcart->productid?>';
                  document.cartform.operate.value='1';"/>
		
		
		&nbsp; <input type="submit" class="button" value="Remove" style="border-style:none;color:#D93008;background-color:#fff;font-size:12px;font-weight:bold;"
		onClick ="javascipt:document.cartform.bookid.value='<?=$rowcart->productid?>';
                            document.cartform.operate.value='2';"/>
	...
	<BR/>
	  ...
	  onClick ="javascipt:document.cartform.operate.value='3';"/>
	  
	  <SPAN style="font-size: 14px;FONT-WEIGHT:BOLD;color:366696;">

 &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<a href="http://cs-server.usc.edu:4551/mtpress/index.php/customer/checkout" style="text-decoration:none"> <INPUT type="button" value="Proceed to Checkout >" /> </a>
	</form>
