...
<%
	...
	try {
		...
		try {
			...
			//String sql= "SELECT * FROM sale WHERE dateOfOrder BETWEEN '"+startDate+"' AND '"+endDate+"' order by dateOfOrder, productCategory";
			String sql="select o.username,c.city,c.state,c.zip,p.productCategory,o.productname,sum(o.quantity) quantity,p.actualPrice,o.orderdate,sum(p.actualPrice*quantity) totalprice  from product p, orderdetail o ,customer c where o.orderdate BETWEEN '"+startDate+"' and '"+endDate+"' and p.productId = o.productid and o.username = c.username  group by p.productCategory,o.productname,c.city,c.state,c.zip";
     ...
     // ------------- Get  Results ------------------------------------->
      ...
      while (rs.next()) {
			...
			if(username[0] == null)
			{
				statusMessage = statusMessage+"No Products Were Sold In This Date Range";
			}
	    rs.close();
...
<table>
	...
	border-style: solid;" width = "963" class = "shoptable">
		...
		<form name ="dreport" id = "dreport" action="manager.jsp" method="post">
		...
		<tr>
					...
					<td align="center"><b><%out.println(dateOfOrder[j]);%></b></td>
		...
		<% } %>
			...
			<tr>
					<td align="center" colspan = "4"><input type="submit" value= "DONE"></td>
...
String fail = "http://http://cs-server.usc.edu:9007/sessionLogin.jsp";
